...
import { Col, Row, Input, Calendar, TimePicker, Select } from "antd";
...
export default function LogCardioWorkout() {
  console.log("進入logCardio..................................");

  const { cardioFilterOptions } = useContext(Content);
  const activityOptions = cardioFilterOptions.map((option) => ({
    value: option,
    label: option,
  }));
    ...
    <div style={{ fontSize: "16rem", textAlign: "center" }}>
      <div>
        ...
        >
          ...
          <style jsx="true">{`
            .ant-select-selection-item {
              font-size: 14rem !important; /* 更改下拉选项字体大小 */
            }
          `}</style>
          <Col>
            <div>DISTANCE</div>
        ...
        >
          ...
          <Col>
            <div>{"DURATION"}</div>
            <div style={{ fontSize: "12rem" }}>{"HH/MM/SS"}</div>
            <Input
              variant="borderless"
              maxLength={36}
              style={{
