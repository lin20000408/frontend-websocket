...
export default function SetLoop({ boxType }) {
  const {
    squares,
    setSquares,
    // Connect
  ...
  const [loopName, setLoopName] = useState(findValue[0]?.data?.name || "");



  const [repetitions, setRepetitions] = useState(findValue[0]?.data?.repetitions || "");
