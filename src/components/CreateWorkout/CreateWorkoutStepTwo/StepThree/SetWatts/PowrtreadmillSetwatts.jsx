...
export default function PowrtreadmillSetwatts({ boxType }) {
  const {
    squares,
    setSquares,
    // Connect
  ...
  const [watts, setWatts] = useState(findValue[0]?.data?.watts || "");
  ...
  console.log();
  const originalSpeed = findValue[0]?.data?.speed;

  const [speed, setSpeed] = useState(
    originalSpeed ? originalSpeed : originalSpeed === 0 ? 0 : "",
  );

  useEffect(() => {
    setSelectedOption(findValue[0]?.data?.functionMode === "push" || "");
  }, []);
  ...
  const handleOKClick = () => {
    ...
    const programUpdateData = {
      ...
      functionMode: selectedOption ? "push" : "run",
      speed: speed,
    }; 
          ...
          <div className="mt-[18rem] h-[455rem] w-[293rem] bg-white ">
            ...
            <div className="mb-[30rem]">
              ...
              <Row justify={"center"} className="">
                <Col
                  className="flex h-[calc(42rem)] w-[calc(120rem)] items-center justify-center text-[calc(14rem)]"
                  style={{
                    border: "2rem solid #84bd00",
                  }}
                >
                  <div
                    className={`font-semibold ${selectedOption ? "none" : styles.signUpUnits}`}
                    style={{
                      display: "flex",
                      justifyContent: "center",
                      alignContent: "center",
                    }}
                    onClick={() => toggleUnit("run")}
                  >
                    <span>RUN</span>
                  </div>
                </Col>
                ...
                <Col
                  // span={10}
                  className="flex h-[calc(42rem)] w-[calc(120rem)] items-center justify-center text-[calc(14rem)]  font-semibold "
                  style={{
                ...
                >
                  <div
                    className={selectedOption ? styles.signUpUnits : "none"}
                    style={{
                      display: "flex",
                      justifyContent: "center",
                      alignContent: "center",
                    }}
                    onClick={() => toggleUnit("push")}
                  >
                    <span> Push</span>
                  </div>
                </Col>
