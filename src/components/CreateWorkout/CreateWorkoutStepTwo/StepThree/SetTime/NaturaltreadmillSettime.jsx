...
export default function NaturaltreadmillSettime({ boxType }) {
  const {
    squares,
    setSquares,
    // Connect
...
const OriginalSecond =findValue[0]?.data?.duration_sec
  ...
  const inclineOriginal=findValue[0]?.data?.incline
  const [incline, setIncline] = useState( inclineOriginal ? inclineOriginal : (inclineOriginal === 0 ? 0 : ""));
